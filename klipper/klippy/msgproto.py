...
DefaultMessages = {
    ...
    # "ghead_temp gh_ptr=%c value=%hu clock=%u": 92,
    # "ghead_location_s gh_ptr=%c location=%c clock=%u": 91,
    # "ghead_ksensor_s gh_ptr=%c sensor=%c clock=%u": 90,
    # "ghead_in_s gh_ptr=%c ghead_s=%c clock=%u": 89,
    # "ghead_fanset_s gh_ptr=%c f_speed=%u clock=%u": 88,
    # "ghead_respond_s gh_ptr=%c mode=%c val=%c clock=%u": 87,

    #setting
    "identify offset=%u count=%c": 1,
    "remote_reset ": 14,
    # "ghead_set_fan_speed ptr=%c fans=%u": 32,
    # "ghead_start_monitor_ghead ptr=%c": 22,
    # "ghead_set_heater_s ptr=%c h_s=%u": 20,
    # "ghead_set_normal_cmd mode=%c ptr=%c h_s=%u": 58,
}
...
class MessageParser:
    ...
    def create_command(self, msg):
        ...
        return cmd
    def check_msg_cmd(self):
        for tag,msg ,format in self.messages:
            if format.startswith("ghead"):
                print (tag,msg,format)
        pass
    def load_user_defined_cmd(self):
        dir = "out/klipper.dict"
        ...
        if isinstance(dict_data, dict):
            for key,val in dict_data.items():
                if key == "responses" or key == "commands":
                    msg_list = eval(str(val))
                    if isinstance(msg_list, dict):
                        #print (key)
                        for msg_key,msg_v in msg_list.items(): 
                            if msg_key.startswith("ghead"):
                                DefaultMessages[msg_key] = msg_v
        else:
            print ("file not dict")
