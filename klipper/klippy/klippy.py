...
Printer is shutdown
"""
default_config_dir = "/opt/Raise3D/klipper-master/printer_0_2.cfg"
class Printer:
    ...
    def __init__(self, main_reactor, bglogger, start_args):
        ...
        if 'config_file' in start_args:
            self.config_dir = start_args['config_file']
        else:
            self.config_dir = default_config_dir
        try:
            self.cur_config.read(self.config_dir)
        except:
            raise self.config_error(
                "fail to load '%s'" % (self.config_dir,))
    ...
    def _save_config(self,section,name,value,gcmd = None):
        if self.cur_config.has_section(section):
            self.cur_config.set(section,name,value)
            with open(self.config_dir,'wb') as configfile:
                self.cur_config.write(configfile)
        if gcmd != None:
            gcmd.respond_raw("save ok,please restart klipper to active changes!")
        pass
    ...
    def set_software_version(self,version):
        self.start_args['software_version'] = version
    def get_software_version(self):
        return self.start_args['software_version']
...
def main():
    ...
    start_args['software_version'] = raise3d_version
    start_args['cpu_info'] = raise3d_version
