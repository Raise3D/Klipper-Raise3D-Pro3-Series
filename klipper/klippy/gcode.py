...
class GCodeDispatch:
    ...
    def __init__(self, printer):
        ...
        handlers = ['M110', 'M112', 'M115', 'M291','M9999','MTEST','M',
    ...
    def register_command(self, cmd, func, when_not_ready=False, desc=None):
        ...
        if not self.is_traditional_gcode(cmd):
            if cmd != 'M' and cmd != 'MTEST':
                origfunc = func
                func = lambda params: origfunc(self._get_extended_params(params))
        #print ("register cmd == " + cmd)
    ...
    def _handle_ready(self):
        self.respond_raw("ok gcode is ready!!")
        logging.debug("gcode is ready!!")
        ...
        # if mcu != None:
        #     mcu.ghead.start_monitor_ghead(0)
        #     print ("try to start monitor ghead")
    # Parse input into commands
    args_r = re.compile('([A-Z_]+|[A-Z*/])')
    def _process_commands(self, commands, need_ack=True):
        for line in commands:
            # Ignore comments and leading/trailing spaces
            org_cmd = str(line).strip()
            # self.respond_raw("rec cmd = %s"%org_cmd)
            if org_cmd == "M5100 RESET" or org_cmd == "MCU RESET":
                self.respond_raw("try reset mcu")
            ...
            if cpos >= 0:
                # report layer
                layer_str = line[cpos:]
                if layer_str.startswith(";LAYER:") or layer_str.startswith("LAYER:") :
                    layer = layer_str.split(':')
                    if len(layer) > 1:
                        self.respond_raw("Current layer:%s"%(layer[1],))
                line = line[:cpos] 
            # Break line into parts and determine command
            if ('\"' in line and line.startswith('M')) and not line.startswith('M291'):
                self.respond_raw("lets do new cmd")
                cparams = {}
                ...
                if res:
                    ...
                    str_need = str_list[1]
                    str_cmd = str_list[0]
                    cparams[str_cmd] = str_need
                    cmdline = line.replace(res[0], "").split(" ")
                    cmd = cmdline[0]
                    for cmdl in cmdline:
                        if '"' not in cmdl:
                            cmd_para = cmdl[:1].upper()
                            cmd_val  = cmdl[1:].upper()
                            cparams[cmd_para] = cmd_val
                            pass
                        else:
                            cmd_para = cmdl[:1].upper()
                            cmd_val = cmdl[1:].strip('"')
                            cparams[cmd_para] = cmd_val
                            pass
                        params = cparams
                else:
            ...
            try:
                ...
                if "Must home axis first" in str(e):
                ...
                if not need_ack:
                    raise
            gcmd.ack()
    def auto_print_background(self,gcode):
        pass
    ...
    def respond_info(self, msg, log=True):
        ...
        self.respond_raw("\n".join(lines))
        #self.respond_raw(lines+"\n")
    def _respond_warning(self, msg):
        logging.warning(msg)
        lines = msg.strip().split('\n')
        if len(lines) > 1:
            self.respond_info("\n".join(lines), log=False)
        self.respond_raw('Warning: %s' % (lines[0].strip(),))
    def _respond_error(self, msg):
        logging.warning(msg)
        lines = msg.strip().split('\n')
        if len(lines) > 1:
            self.respond_info("\n".join(lines), log=False)
        if lines[0].startswith("Printer is not ready"):
            self._respond_warning(lines[0])
        else:
    ...
    def cmd_M9999(self,gcmd):
        if 'S' in gcmd._params:
            val = int(gcmd._params['S'])
            if val == 0 :
                 mcu = self.printer.lookup_object('mcu')
                 if mcu != None:
                     mcu.debug_msgproto()  
                ...
                rail = kin.get_rail('x')
                if rail != None:
                    endstop = rail.get_mcu_endstop()
                    print_time = toolhead.get_last_move_time()
                    res = endstop.query_endstop(print_time)
                    gcmd.respond_raw("X endstop status is %s"%str(res))
                else:
                    gcmd.respond_raw("x axes cant find")
            elif val == 3:
                # see x endstop 
                toolhead = self.printer.lookup_object('toolhead')
                kin = toolhead.kin
                ...
                if rail != None:
                    ...
                    print_time = toolhead.get_last_move_time()
                    res = endstop.query_endstop(print_time)
                    gcmd.respond_raw("Y endstop status is %s"%str(res))
                else:
                    gcmd.respond_raw("y axes cant find")
            #print ("try to start monitor ghead")
        self.respond_raw("ok get"+str(gcmd._params))
    def cmd_M999(self,gcmd):
        # try to set moveboard into bossac
        self.request_restart('firmware_restart')
        self.respond_raw("ok")
    def cmd_M106(self, gcmd):
        self.respond_raw("ok get"+gcmd._command)
        mcu = self.printer.lookup_object('mcu')
        if mcu != None:
            mcu.ghead.set_fanspeed_cmd(0,50)
    def cmd_M(self,gcmd):
        self.respond_raw("ok")
    def cmd_M291(self,gcmd):
        self.respond_raw(str(gcmd._commandline))
        cmd = str(gcmd._commandline).split(" ",1)
        res = re.findall(r"\"(.*)\"", gcmd._commandline)
        if res:
            self.respond_raw(res[0])
        elif len(cmd) > 1 and cmd[1].startswith('P'):
            if "\"" in cmd[1]:
                pcmd = str(cmd[1]).split("\"")
                if len(pcmd) > 1:
                   self.respond_raw(pcmd[1]) 
    def cmd_MTEST(self,gcmd):
        if 'S' in gcmd._params:
            val = int(gcmd._params['S'])
            if val == 0 :
                 mcu = self.printer.lookup_object('mcu')
    ...
    def cmd_M115(self, gcmd):
        ...
        self.run_script_from_command("M5100 G0 CV")
        self.run_script_from_command("M5100 G0 V")
...
class GCodeIO:
    def __init__(self, printer):
        ...
        self.timer_handle = self.reactor.register_timer(self.resend, self.reactor.NEVER)
        self.msg_buff = []
        self.msg_is_resending = False
        self.last_msg_resend = None
        self.resend_count = 0
    def _handle_ready(self):
        self.is_printer_ready = True
    ...
    def _process_data(self, eventtime):
        ...
        # print ('recv data1--->'+data)
        self.bytes_read += len(data)
        lines = data.split('\n')
        lines[0] = self.partial_input + lines[0]
        self.partial_input = lines.pop()
        # print ('partial_input----->',self.partial_input)
        pending_commands = self.pending_commands
        # print ('pending_commands----->',pending_commands)
        ...
        if self.fd_handle is None:
            self.fd_handle = self.reactor.register_fd(self.fd,
                                                      self._process_data)
    def resend(self,eventtime):
        logging.exception("try to resend..")
        if len(self.msg_buff) > 0:
            msg = self.msg_buff[0]
            if self.last_msg_resend == msg:
                self.resend_count = self.resend_count + 1
                if self.resend_count >= 2:
                    self.resend_count = 0
                    self.pipe_is_active = True
            else:
                self.last_msg_resend = msg
                self.resend_count = 0
            res = self._respond_raw(msg)
            if res != 'ok':
            ...
            logging.exception("no msg to been resent..")
            return self.reactor.NEVER
        pass
    def _respond_raw(self, msg):
        if self.pipe_is_active:
            try:
                ...
                logging.exception("Write g-code response [%s]" % (msg))
