...
class MCU_stepper:
    ...
    def generate_steps(self, flush_time):
        ...
        if ret:
            raise error("Internal error in stepcompress %d"%ret)
...
class PrinterRail:
    def __init__(self, config, need_position_minmax=True,
        ...
        self.mcu_endstop =  mcu_endstop = self.endstops[0][0]
    ...
    def set_position_max(self,pos,gcmd = None):
        if pos != self.position_max and pos > self.position_min:
            self.position_max = pos
    def set_position_min(self,pos,gcmd = None):
        if pos != self.position_min and pos < self.position_max:
            self.position_min = pos
    def check_endstop(self):
        return self.has_endstoppin
    ...
    def add_extra_stepper(self, config):
        ...
        if endstop_pin == None:
            #print (stepper.get_name(),"has no endstop_pin")
            self.has_endstoppin = False
            return
        else:
            #print (stepper.get_name(),"has endstop_pin")
            self.has_endstoppin = True
