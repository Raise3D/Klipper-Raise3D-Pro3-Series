...
#include <fcntl.h>
void print_len_hex(uint16_t len,uint8_t *buf)
{
    char sbuf[256] = { 0 };
    if(len >= 60)
        return;
    ...
    {
        sprintf(sbuf,"%s%2X ",sbuf,buf[i]);
    }
    printf("%s\r\n",sbuf);
}
void debug_len_hex(unsigned char *dat,unsigned int len)
{
...
{
    if (sq->serial_fd_type != SQT_CAN) {
        pthread_mutex_lock(&sq->mlock);
        int ret = write(sq->serial_fd, buf, buflen);
        //print_len_hex(buflen,buf);
        //debug_len_hex((unsigned char *)buf,buflen);
        pthread_mutex_unlock(&sq->mlock);
        if (ret < 0)
    ...
    while (buflen) {
        ...
        //debug_len_hex((unsigned char *)&cf,sizeof(cf));
        if (ret < 0) {
...
serialqueue_alloc(int serial_fd, char serial_fd_type, int client_id)
{
    ...
    // Thread setup
    ret = pthread_mutex_init(&sq->mlock, NULL);
    if (ret)
        goto fail;
                       ...
                       , struct list_head *msgs)
{
    ...
    // if(qm->msg[0] == 0x48)
    // {
    //     print_len_hex(qm->len,qm->msg);
    //     printf("min_clock = %ld,need_kick_clock = %ld\r\n",(uint64_t)qm->min_clock ,(uint64_t)sq->need_kick_clock);
    //     if (qm->min_clock < sq->need_kick_clock) {
    //         printf("need wake bgthread\r\n");}
    // }
