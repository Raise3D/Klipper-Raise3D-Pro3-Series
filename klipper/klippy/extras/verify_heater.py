...
class HeaterCheck:
    ...
    def check_event(self, eventtime):
        ...
        current_error = abs((target - self.hysteresis) - temp)
        if not self.approaching_target:
            if target != self.last_target:
                ...
                if current_error >= 15:
                    self.cur_error_count = self.cur_error_count + 1
                    gcode = self.printer.lookup_object('gcode')
                    gcode._respond_warning("%s temp drop down to %.3f last temp %.3f target temp %.3f"%(self.heater_name,temp,self.last_temp,target))
                    if self.cur_error_count > 10:
                        gcode._respond_error("%s temp drop down to %.3f last temp %.3f target temp %.3f"%(self.heater_name,temp,self.last_temp,target))
                        self.printer.send_event("temperature:error")
                else:
                    self.cur_error_count = 0
                    #MAX ERROR = 120
                if self.error >= self.max_error:
                    ...
                    if self.error_count > 5 :
                        ...
                        logs = "heater name = %s ctemp = %0.3f ttemp = %0.3f gtemp = %0.3f cu = %0.3f"%(self.heater_name,temp,target,self.heating_gain,self.goal_temp)
                        gcode._respond_warning(logs)
