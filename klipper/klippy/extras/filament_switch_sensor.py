...
class RunoutHelper:
    def __init__(self, config):
        ...
        self.timer_handle = self.reactor.register_timer(
                self.shake_handle, self.reactor.NEVER)
        # Read config
    ...
    def note_filament_present(self, is_filament_present):
        ...
        #notify other module related filament fuction
        if self.filament_present == 1:
            self.printer.send_event("filament:in", self.name)
        else:
            self.printer.send_event("filament:out", self.name)
        ...
        self.reactor.update_timer(self.timer_handle,eventtime + 1.)
    ...
    def shake_handle(self,eventtime):
        if self.filament_present:
            msg = "Filament Sensor %s: filament detected" % (self.name)
            # filament in
        else:
