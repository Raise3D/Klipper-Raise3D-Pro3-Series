...
class GCodeMove:
    def __init__(self, config):
        ...
        printer.register_event_handler("extruder:do_before_active",
                                       self._do_before_activate_extruder)
        ...
        self.filament_state = {"F0":False, "F1":False}
        ...
        self.cur_offset = []
        self.active_extruder_pos = []
        self.active_extruder_retact_dist = 4.0
        self.last_extruder = None

    def filament_out_event(self, filament_n):
        self.filament_state[filament_n] = False
        self.nprintf("gcode move filament is running out %s"%(str(filament_n)))
    
    def filament_in_event(self, filament_n):
        self.filament_state[filament_n] = True
        self.nprintf("gcode move filament is running in %s"%(str(filament_n)))
    ...
    def _do_before_activate_extruder(self):
        self.active_extruder_pos = []
        self.active_extruder_pos = self._get_gcode_position()
        zpos = self.active_extruder_pos[2] + self.active_extruder_retact_dist 
        # self.nprintf("do before active new extruder,cur z pos is %.2f %.2f"%(self.active_extruder_pos[2], zpos))
        # self.nprintf(str(self.base_position))
        self.gcode.run_script_from_command("G1 Z%.2f"%zpos)
        toolhead = self.printer.lookup_object('toolhead')
        toolhead.wait_moves()
        self.last_extruder = toolhead.get_extruder()
    def _handle_activate_extruder(self):
        toolhead = self.printer.lookup_object('toolhead')
        cur_extruder = toolhead.get_extruder()
        if cur_extruder != None:
            self.extrude_factor = cur_extruder.get_extruder_factor() / 100.
            #self.cur_offset = cur_extruder.get_extruder_factor()
        else:
            self.extrude_factor = 1.
        self.reset_last_position()
        #self.extrude_factor = 1.
        self.base_position[3] = self.last_position[3]
        # self.nprintf("do active new extruder,cur z pos is %.2f"%self.active_extruder_pos[2])
        if len(self.active_extruder_pos) > 2:
            self.gcode.run_script_from_command("G1 Z%.2f"%(self.active_extruder_pos[2]))
    ...
    def get_extruder_factor(self):
        ...
        for i in range(4):
            ...
            if extruder != None:
                out += "F%d:%.1f "%(i,extruder.get_extruder_factor())
            else:
                break
        return out
    def get_speed_factor(self):
        return "P:%.1f "%(self.printer_speed_factor,)
        ...
        try:
            for pos, axis in enumerate('XYZ'):
                if axis in params:
                    ...
                    if axis == 'Z':
                        move_pos = self._get_gcode_position()
                        self.nprintf("ZMove:%.2f" % (move_pos[pos]))
                    # self.nprintf("cur offset %s"%(str(offset)))
                    # self.nprintf("move to %s %s %s"%(str(self.last_position), str(self.base_position[pos]), str(v)))
            if 'E' in params:
    ...
    def checkFilamentState(self) :
        toolhead = self.printer.lookup_object('toolhead')
        active_extruder = toolhead.get_extruder()
        if ((active_extruder.get_name() == 'extruder' and not self.filament_state['F0'])) :
            msg = "Filament Sensor %s: filament not detected" % ('F0')
            self.nprintf(msg)
        elif ((active_extruder.get_name() == 'extruder1' and not self.filament_state['F1'])) :
            msg = "Filament Sensor %s: filament not detected" % ('F1')
            self.nprintf(msg)
    ...
    def cmd_M220(self, gcmd):
        ...
        self.printer_speed_factor = gcmd.get_float('S', 100., above=0.)
    ...
    def cmd_M221(self, gcmd):
        ...
        if 'S' not in cmdlist:
            gcmd._respond_warning("ivalid parameters when set extrude factor")
            return 
        if 'T' in cmdlist:
            extruder_n = int(cmdlist['T'])
            if extruder_n == 0:
                extruder_name = "extruder"
            else:
                extruder_name = "extruder%d"%(extruder_n)
            extruder = self.printer.lookup_object(extruder_name)
            if extruder != None:
                extruder.set_extruder_factor(float(cmdlist['S']))
            if extruder == cur_extruder:
            ...
            cur_extruder.set_extruder_factor(float(cmdlist['S']))
            new_extrude_factor = gcmd.get_float('S', 100., above=0.) / 100.
            last_e_pos = self.last_position[3]
            e_value = (last_e_pos - self.base_position[3]) / self.extrude_factor
            self.base_position[3] = last_e_pos - e_value * new_extrude_factor
    ...
    def cmd_SET_GCODE_OFFSET(self, gcmd):
        ...
        for pos, axis in enumerate('XYZE'):
            ...
            # self.nprintf("set gcode offset %s:%.2f %.2f %.2f %.2f"%(axis, offset, self.homing_position[pos], self.base_position[pos], move_delta[pos]))
