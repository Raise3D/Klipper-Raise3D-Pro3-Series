...
class SafeZHoming:
    ...
    def check_axes_retract(self,toolhead,axes,gcmd = None):
        if toolhead != None:
            pos_ptr = 0
            if axes.lower() == 'y':
                pos_ptr = 1
            kin = toolhead.kin
            rail = kin.get_rail(axes.lower())
            if rail != None :
                ...
                if int(res) == 1:
                    if gcmd != None:
                        gcmd.respond_raw("%s is hitting a endstop just restract 20mm"%axes)
                    pos = toolhead.get_position()
                    pos[pos_ptr] = 0
                    toolhead.set_position(pos, homing_axes=[pos_ptr])
                    if pos_ptr == 0:
                        toolhead.manual_move([25.0, None, None],100)
                    else:
                        toolhead.manual_move([None, 25.0, None],100)
    def cmd_G28(self, gcmd):
        toolhead = self.printer.lookup_object('toolhead')

        # if xy axes is hitting endstop pin just restact 2cm
        # self.check_axes_retract(toolhead,'x',gcmd)
        ...
        if self.z_hop != 0.0:
            ...
            self.gcode.run_script_from_command("M5100 G0 Z1")
        ...
        if need_z:
            ...
            pos = toolhead.get_position()
            # gcmd.respond_raw("cur pos %s"%(str(pos)))
            # gcmd.respond_raw("z hop %.2f"%self.z_hop)
            if self.z_hop:
                #self.gcode.run_script_from_command("G1 Z10")
