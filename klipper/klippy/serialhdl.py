...
class SerialReader:
    BITS_PER_BYTE = 10.
    def __init__(self, reactor, printer,warn_prefix=""):
        ...
        self.reset_cmd_queue = self.alloc_command_queue()
        ...
        self.remote_reset_delay_timer = self.reactor.register_timer(
                self.reset_mcu_handle, self.reactor.NEVER)
    def reset_mcu_handle(self,eventtime):
        self.try_remote_reset()
    ...
    def raw_send_wait_ack(self, cmd, minclock, reqclock, cmd_queue):
        ...
        if params is None:
            ...
            self.reactor.update_timer(self.remote_reset_delay_timer,eventtime + 0.5)
            # self._error("Serial connection closed")
        return params
    def try_remote_reset(self):
        logging.warn("try to reset mcu")
        cmd = self.msgparser.create_command("remote_reset")
        self.raw_send(cmd, 0, 0, self.reset_cmd_queue)
        pass
