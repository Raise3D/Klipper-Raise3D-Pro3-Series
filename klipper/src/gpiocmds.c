...
digital_load_event(struct timer *timer)
{
    ...
    {
        // mprintf(SERIAL2_PTR,"Missed scheduling of next digital out event!\r\n");
    ...
    // mprintf(SERIAL2_PTR,"update end_time = %d max_duration = %d\r\n",d->end_time,d->max_duration);
    // Schedule next event
...
command_queue_digital_out(uint32_t *args)
{
    ...
    // mprintf(SERIAL2_PTR,"curtime = %d end_time = %d,pwm_time = %d\r\n",d->end_time,time);
    if (flags & DF_CHECK_END && timer_is_before(d->end_time, time))
    {
        //mprintf(SERIAL2_PTR,"try shutdown but not!\r\n");
        // mprintf(SERIAL2_PTR,"try shutdown ok..%d\r\n",args[0]);
        shutdown("Scheduled digital out event will exceed max_duration---");
    }
