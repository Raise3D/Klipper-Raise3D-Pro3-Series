...
/* only for some special cmd id */
uint8_t no_chenkid_list[] = {21,35};
uint8_t is_need_check(uint8_t id)
{
    uint8_t size = sizeof(no_chenkid_list);
    for(int i = 0; i < size ;i++)
    {
        if(no_chenkid_list[i] == id)
            return 0;
    }
    return 1;
}
...
command_sendf(const struct command_encoder *ce, ...)
{
    ...
    writeb(&in_sendf, 0);
	//mprintf(DEBUG_PORT,"send id = %d",ce->msg_id);
}
...
command_dispatch(uint8_t *buf, uint_fast8_t msglen)
{
    ...
    while (p < msgend) {
        ...
        if (sched_is_shutdown() && !(READP(cp->flags) & HF_IN_SHUTDOWN)) {
            ...
            klipper_handle_response(SHUTDOWN_EVENT_FROM_KLIPPER);
            continue;
        }
        irq_poll();
        ...
        func(args);
        klipper_handle_response(NORMAL_MSG_FROM_KLIPPER);
        //mprintf(DEBUG_PORT,"cmd recv %d\r\n",cmdid);
    }
}
